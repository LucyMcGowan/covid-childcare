function(input, output, session) {
  ...
  camps <- d %>%
    filter(`Open?` == "Yes") %>%
    filter(grepl("Camp", `Type of Institution`, ignore.case = TRUE)) %>%
    group_by(`Week of`) %>%
    summarise(
      `Number of Locations` = sum(
        !is.na(`Number of Children`) |
  ...
  output$camps <- renderDataTable(camps,
                                  rownames = FALSE,
                                  escape = FALSE,
                                  options = list(
                                    ordering = FALSE,
  ...
  output$large <- renderDataTable(large,
                                  rownames = FALSE,
                                  escape = FALSE,
                                  options = list(
                                    ordering = FALSE,
